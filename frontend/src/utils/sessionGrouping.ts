...
// "desc" = mais recentes primeiro (default)
// "asc"  = mais antigas primeiro
// "size" = mais eventos primeiro (sessões "maiores")
export type SessionSortDirection = "desc" | "asc" | "size";
...
export function groupSessionsByProject(
  ...
  const compareGroups = (
    ...
    if (direction === "size") {
      const aSum = a.reduce((acc, s) => acc + s.eventCount, 0);
      const bSum = b.reduce((acc, s) => acc + s.eventCount, 0);
      return bSum - aSum;
    }
    const sign = direction === "desc" ? 1 : -1;
    const aExtreme =
  ...
  const sorted = [...groups.entries()].sort(compareGroups);
