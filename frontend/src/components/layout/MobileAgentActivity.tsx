...
export function MobileAgentActivity({
  ...
  return (
    <div className="flex-[2] bg-jp-ink border border-jp-divider-soft rounded-lg overflow-hidden min-h-0">
      <div className="bg-jp-surface-1 px-3 py-2 border-b border-jp-divider-soft flex items-center gap-2">
        ...
        <span className="text-jp-fg-dim text-xs">({agents.size})</span>
      </div>

      <div className="overflow-y-auto h-[calc(100%-36px)] p-2">
        {/* Boss Status */}
        <div className="mb-3 p-2 bg-jp-surface-1/50 rounded-lg border border-amber-500/30">
          <div className="flex items-center gap-2 mb-1">
            <div className="w-2 h-2 rounded-full bg-amber-500" />
            <span className="text-amber-400 font-bold text-xs">
              {t("mobile.boss")}
            </span>
            <span className="text-jp-fg-dim text-[10px] font-mono ml-auto">
              {boss.backendState}
            </span>
          </div>
          {boss.currentTask && (
            <p className="text-jp-fg-muted text-[11px] truncate">
              {boss.currentTask}
            </p>
          )}
          {boss.bubble.content && (
        ...
        {/* Agent List */}
        {agents.size === 0 ? (
          <div className="text-center text-jp-fg-dim text-xs italic py-4">
          ...
          <div className="flex flex-col gap-2">
            {Array.from(agents.values()).map((agent) => (
              <div
                key={agent.id}
                className="p-2 bg-jp-surface-1/50 rounded-lg border border-jp-divider-soft"
              >
                <div className="flex items-center gap-2 mb-1">
                  <div
                  ...
                  />
                  <span className="text-jp-fg font-bold text-xs">
                    {agent.name}
                  </span>
                  <span className="text-jp-fg-dim text-[10px] font-mono ml-auto">
                    {agent.phase}
